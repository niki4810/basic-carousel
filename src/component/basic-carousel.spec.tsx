...
const TestCarouselComponet: FC<Partial<BasicCarouselProps>> = ({onSlideChange, showPaginationButtons, showPaginationDots, paginationButtonsPosition}) => {
  const tiles = new Array(7).fill("").map((_, i) => `Tile - ${i}`);

  return (
    <div style={{ width: "1300px", height: "800px" }}>
      <BasicCarousel onSlideChange={onSlideChange} showPaginationButtons={showPaginationButtons} showPaginationDots={showPaginationDots} paginationButtonsPosition={paginationButtonsPosition}>
        {tiles.map((tile, i) => {
          return (
            <div
              key={tile}
...
describe("When the carousel is rendered", () => {
  ...
  beforeEach(async () => {
    ...
    renderResult = await render(<TestCarouselComponet onSlideChange={slideChangeCb}/>);
  });
  ...
  it("should hide pagination button container when showPaginationButtons prop is set to false", () => {
    renderResult.rerender(<TestCarouselComponet showPaginationButtons={false} />);
    expect(screen.getByTestId("previous-btn-container")).toHaveClass("basic_carousel__btn--container-no-width");
    expect(screen.getByRole("button", {name: "<"})).not.toHaveClass("basic_carousel__btn--active");
    expect(screen.getByTestId("next-btn-container")).toHaveClass("basic_carousel__btn--container-no-width");
    expect(screen.getByRole("button", {name: ">"})).not.toHaveClass("basic_carousel__btn--active");
  });
  ...
  it("should hide pagination dots container when showPaginationDots prop is set to false", () => {
    ...
    expect(screen.queryByTestId("pagination-dots-container")).not.toBeInTheDocument();
  });

  it("should apply correct classes when the paginationButtonsPosition is set to onTop", () => {
    renderResult.rerender(<TestCarouselComponet paginationButtonsPosition={PaginationButtonPositions.onTop} />);

    expect(screen.getByTestId("previous-btn-container")).toHaveClass("basic_carousel__btn--container-no-width");
    expect(screen.getByRole("button", {name: "<"})).toHaveClass("basic_carousel__btn--positioned");
    expect(screen.getByTestId("next-btn-container")).toHaveClass("basic_carousel__btn--container-no-width");
    expect(screen.getByRole("button", {name: ">"})).toHaveClass("basic_carousel__btn--positioned");
  });
});
